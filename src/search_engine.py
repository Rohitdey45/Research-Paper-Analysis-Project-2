...
class PaperSearchEngine:
    ...
    def __init__(
        ...
        if not os.path.exists(CLEANED_CSV_PATH) or not os.path.exists(INDEX_PATH):
            raise FileNotFoundError(
                ...
                f"Expected files: {CLEANED_CSV_PATH} and {INDEX_PATH}"
            )
    ...
    def summarize(self, text: str, max_length: int = 120, min_length: int = 40) -> str:
        ...
        dynamic_min = min(min_length, max(12, word_count // 4))
        dynamic_max = min(max_length, max(dynamic_min + 10, word_count // 2))
    ...
    def full_report(
        ...
        for r in results:
            if include_summary and self.summarizer is not None:
                r["summary"] = self.summarize(r["abstract"])
            if include_keywords and self.kw_model is not None:
                r["keywords"] = self.extract_keywords(
                    r["abstract"],
                    top_n=keyword_count,
                )
        return results
